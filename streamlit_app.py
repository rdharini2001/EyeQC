...
def display_interactive_plots(df):
    ...
    # --- Additional, expanded interpretation ---
    st.markdown("""
    **Extended Clinical Interpretation**:  
    - **Purpose**: Parallel coordinates visually compare all your chosen metrics in one plot.  
    - **Clinical Use**: Consistent high lines across multiple axes suggest an overall high-quality image, suitable for detailed diagnosis. 
      Images with low dips (especially in blur or contrast) may require retakes or further investigation.  
    - **Key Insight**: Outliers in a particular metric axis can point to specific quality issues (e.g., overly dark images if mean intensity is very low).
    """)
...
def display_static_visualizations(df):
    ...
    if len(df_plot) > 1:
        ...
        st.markdown("""
        **Extended Clinical Interpretation**:  
        - **Purpose**: This grid lets you see how each pair of metrics correlates (positive, negative, or no correlation).  
        - **Clinical Use**: If blur is strongly negatively correlated with contrast, it implies blurry images often lack contrast—important for diagnosing certain conditions.  
        - **Key Insight**: Clusters in the scatter plots can reveal groups of images that share similar characteristics (e.g., underexposed or overexposed sets).
        """)
    else:
    ...
    **Clinical Interpretation**:  
    - **Purpose**: A violin plot combines a box plot with a density plot, showing both the distribution and quartiles of each metric.  
    - **Meaning for Clinicians**: Wider areas in the violin indicate more images have that metric value. Narrow areas indicate fewer images.  
    - **Example**: If the **entropy** violin is heavily skewed to one side, it might mean many images have poor textural detail.
    """)
    ...
    if len(df_plot) > 0:
        ...
        st.markdown("""
        **Clinical Interpretation**:  
        - **Purpose**: Compare multiple metrics for a few selected images on a single radial chart.  
        - **Usage**: A well-filled or balanced radar shape suggests a high-quality image across multiple dimensions.  
        - **Example**: If one image has very low “blur” but high “contrast” and “entropy,” it may be a sharp, high-detail image.
        """)
    else:
    ...
    st.markdown("""
    **Clinical Interpretation**:  
    - **Purpose**: A boxen plot provides detailed insight into the distribution (including outliers) of a given metric across Low, Medium, and High quality groups.  
    - **Example**: If the PSNR is significantly lower for the “Low” group, it confirms these images might be too noisy or of poor clarity for diagnostic use.  
    - **Key Insight**: Helps quickly see whether a specific metric strongly differentiates High-quality from Low-quality images.
    """)
    ...
    if metric_left != metric_right and metric_left in df_plot.columns and metric_right in df_plot.columns:
        ...
        st.markdown(f"""
        **Clinical Interpretation**:  
        - **Purpose**: Compares **{metric_left}** and **{metric_right}** together over the same image index.  
        - **Clinical Use**: If both metrics decrease for certain images, it might highlight a specific quality issue (e.g., low contrast + high blur).  
        - **Key Insight**: Aligning two metrics on one chart can reveal simultaneous trends or correlations—for instance, do images with low PSNR also exhibit low contrast?
        """)
    else:
    ...
    if len(plot_df) < 2:
        ...
        st.pyplot(fig)
        # Additional PCA interpretation
        st.markdown("""
        **Clinical Interpretation (PCA)**:  
        - **Purpose**: Reduces multiple metrics into two principal components.  
        - **Clinical Use**: Clusters in PCA space can separate **high-quality** from **low-quality** images, indicating distinct imaging patterns (blur, noise, etc.).  
        - **Key Insight**: If certain images lie far from the main cluster, they may have severe defects or unique artifacts.
        """)
        ...
        st.pyplot(fig)
        # Additional t-SNE interpretation
        st.markdown("""
        **Clinical Interpretation (t-SNE)**:  
        - **Purpose**: t-SNE excels at finding local structure, grouping similar images more distinctly than PCA.  
        - **Clinical Use**: Can reveal sub-clusters of poor-quality images (e.g., motion blur vs. underexposure).  
        - **Key Insight**: Outlier points in t-SNE space might represent rare but extremely poor-quality images unsuitable for diagnosis.
        """)
    ...
    if len(parallel_df) < 2:
        ...
        st.markdown("""
        **Clinical Interpretation**:  
        - **Purpose**: Like the interactive parallel coordinates, but statically showing each image's normalized metrics.  
        - **Clinical Use**: Quickly see which quality group (Low, Medium, High) dominates each axis.  
        - **Key Insight**: Parallel coordinates can highlight specific metrics where “Low” group images consistently dip below the rest, guiding improvements in imaging protocols.
        """)
    ...
    if len(dendro_df) > 1:
        ...
        st.markdown("""
        **Clinical Interpretation**:  
        - **Purpose**: Groups images based on similarity across all metrics.  
        - **Clinical Use**: Identifies clusters of similarly poor (or good) images—useful for diagnosing systemic issues like underexposure across an entire batch.  
        - **Key Insight**: Large branch distances indicate significantly different image quality patterns.
        """)
    else:
    ...
    if all(ax in df_plot.columns for ax in [x_3d, y_3d, z_3d]):
        ...
        st.markdown("""
        **Extended Clinical Interpretation**:  
        - **Purpose**: Provides a customizable 3D view of your chosen metrics.  
        - **Clinical Use**: If a set of images clusters distinctly in 3D space, they may share specific quality traits (e.g., high contrast, low blur).  
        - **Key Insight**: Outliers are easier to spot in 3D; color-coding can highlight which metric (like blur or entropy) is abnormal.
        """)
    else:
...
def display_clinical_visualizations(df):
    ...
    st.markdown("""
    **Clinical Interpretation**:  
    - **Purpose**: Shows distributions (min, max, quartiles) of critical clinical metrics like mean intensity, contrast, blur, etc.  
    - **Usage**: Helps identify typical vs. outlier values for metrics that clinicians rely on (e.g., if blur is typically low except for a few extreme outliers).  
    - **Key Insight**: Outliers in this boxplot might represent images with major artifacts or severe under/overexposure.
    """)
    ...
    st.markdown("""
    **Clinical Interpretation**:  
    - **Purpose**: Indicates how strongly each clinical metric is associated with the others.  
    - **Clinical Use**: If two metrics are highly correlated (e.g., blur and psnr), measuring one may suffice in certain protocols.  
